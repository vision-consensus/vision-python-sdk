...
from visionpy.exceptions import BadKey, BadSignature, BadAddress
...
def public_key_to_base58check_addr(pub_key: bytes) -> str:
    primitive_addr = b"\x46" + keccak256(pub_key)[-20:]
    addr = base58.b58encode_check(primitive_addr)
    return addr.decode()


def public_key_to_addr(pub_key: bytes) -> bytes:
    return b"\x46" + keccak256(pub_key)[-20:]
...
def to_base58check_address(raw_addr: Union[str, bytes]) -> str:
    ...
    if isinstance(raw_addr, (str,)):
        if raw_addr[0] == "V" and len(raw_addr) == 34:
            try:
                # assert checked
            ...
            if raw_addr.startswith("0x"):  # eth address format
                return base58.b58encode_check(b"\x46" + bytes.fromhex(raw_addr[2:])).decode()
            else:
        ...
        if len(raw_addr) == 21 and int(raw_addr[0]) == 0x46:
            return base58.b58encode_check(raw_addr).decode()
        if len(raw_addr) == 20:  # eth address format
            return base58.b58encode_check(b"\x46" + raw_addr).decode()
...
def is_base58check_address(value: str) -> bool:
    return value[0] == "V" and len(base58.b58decode_check(value)) == 21


def is_hex_address(value: str) -> bool:
    return value.startswith("46") and len(bytes.fromhex(value)) == 21
