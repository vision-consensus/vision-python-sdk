...
from visionpy.exceptions import (
    BadSignature,
    BadKey,
    BadHash,
    BlockNotFound,
...
class AsyncTransactionRet(dict):
    def __init__(self, iterable, client: "AsyncVision", method: AsyncContractMethod = None):
...
class AsyncTransaction(object):
    ...
    @classmethod
    async def from_json(cls, data: Union[str, dict], client: "AsyncVision" = None) -> "AsyncTransaction":
    ...
    async def broadcast(self) -> AsyncTransactionRet:
        """Broadcast the transaction to VISION network."""
...
class AsyncTransactionBuilder(object):
    ...
    def __init__(self, inner: dict, client: "AsyncVision", method: AsyncContractMethod = None):
        self._client = client
        self._raw_data = {
            "contract": [inner],
    ...
    def with_owner(self, addr: VAddress) -> "AsyncTransactionBuilder":
    ...
    def fee_limit(self, value: int) -> "AsyncTransactionBuilder":
        """Set fee_limit of the transaction, in `VDT`."""
...
# noinspection PyBroadException
class AsyncVS(object):
    """The VS(transaction) API."""

    def __init__(self, vision):
        self._vision = vision

    @property
    def client(self) -> "AsyncVision":
        return self._vision
    ...
    def transfer(self, from_: VAddress, to: VAddress, amount: int) -> AsyncTransactionBuilder:
        """Transfer VS. ``amount`` in `VDT`."""
    ...
    def asset_transfer(self, from_: VAddress, to: VAddress, amount: int, token_id: int) -> AsyncTransactionBuilder:
        """Transfer VRC10 tokens."""
        return self._build_transaction(
    ...
    def account_permission_update(self, owner: VAddress, perm: dict) -> "AsyncTransactionBuilder":
        """Update account permission.

        :param owner: Address of owner
        :param perm: Permission dict from :meth:`~visionpy.Vision.get_account_permission`
        """
    ...
    def account_update(self, owner: VAddress, name: str) -> "AsyncTransactionBuilder":
    ...
    def freeze_balance(
        ...
        :param resource: Resource type, can be ``"ENTROPY"`` or ``"PHOTON"``
        """
        payload = {
            "owner_address": keys.to_hex_address(owner),
    ...
    def create_witness(self, owner: VAddress, url: str) -> "AsyncTransactionBuilder":
        """Create a new witness, will consume 1_000 VS."""
    ...
    def vote_witness(self, owner: VAddress, *votes: Tuple[VAddress, int]) -> "AsyncTransactionBuilder":
    ...
    def deploy_contract(self, owner: VAddress, contract: AsyncContract) -> "AsyncTransactionBuilder":
...
# noinspection PyBroadException
class AsyncVision(object):
    """The Async VISION API Client.

    :param provider: An :class:`~visionpy.providers.HTTPProvider` object, can be configured to use private node
    :param network: Which network to connect, one of ``"mainnet"``, ``"shasta"``, ``"nile"``, or ``"visionex"``
    """

    # Address API
    is_address = staticmethod(keys.is_address)
    """Is object a VISION address, both hex format and base58check format."""
    ...
    # Account query
    async def get_account(self, addr: VAddress) -> dict:
    ...
    async def get_account_resource(self, addr: VAddress) -> dict:
    ...
    async def get_account_balance(self, addr: VAddress) -> Decimal:
        """Get VS balance of an account. Result in `VS`."""

        info = await self.get_account(addr)
        return Decimal(info.get("balance", 0)) / 1_000_000

    async def get_account_asset_balances(self, addr: VAddress) -> dict:
        """Get all VRC10 token balances of an account."""
        info = await self.get_account(addr)
        return {p['key']: p['value'] for p in info.get("assetV2", {}) if p['value'] > 0}

    async def get_account_asset_balance(self, addr: VAddress, token_id: Union[int, str]) -> int:
        """Get VRC10 token balance of an account. Result is in raw amount."""
    ...
    async def get_account_permission(self, addr: VAddress) -> dict:
    ...
    async def get_asset(self, id: int = None, issuer: VAddress = None) -> dict:
    ...
    async def get_contract(self, addr: VAddress) -> AsyncContract:
        ...
        cntr = AsyncContract(
            ...
            origin_entropy_limit=info.get("origin_entropy_limit", 0),
